# Githup

## At the beginning we should  go to the githup site And create a profile  in the gitup

***We learned several commands in GitHub***

***
*for example*

1. git clone to websiet 
to download the link on the pc

2. git add  
to add file in local git 

3. git push origin main 
push file to the cloud


4. git commit -m ''
to add any Comments and this Comments have a id  






[Githup](http://githup.com/)









 ![Githup](data:image/png;base64,) 